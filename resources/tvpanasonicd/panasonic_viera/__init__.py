...
# import urlopen, Request, HTTPError, build_opener, HTTPHandler
...
class TimeOutError(Exception):
    """This exception is thrown when there is a time out. """

class ForbiddenError(Exception):
    """This exception is thrown when there is a HTTP Forbidden error. """
...
class RemoteControl:
    ...
    def __init__(
        ...
        if self._type == TV_TYPE_NONENCRYPTED:
            ...
            try:
                root = {}
                res = urllib.request.urlopen(url, timeout=self._timeout).read()
    ...
    def soap_request(self, url, urn, action, params, body_elem="m"):
        ...
        if urn == URN_REMOTE_CONTROL and action not in [
            ...
            if None not in [
                ...
                raise EncryptionRequired("Please refer to the docs for using encryption") from None
        ...
        try:
            res = urllib.request.urlopen(req, timeout=self._timeout).read()
        except urllib.request.HTTPError as err:
            ...
            if err.code == 500:
                ...
                for child in xml.iter():
                    if child.tag.endswith("errorDescription"):
                        raise SOAPError(child.text) from None
            elif err.code == 403: # Forbidden
                raise ForbiddenError() from None
            raise ex from None # Pass to the next handler
        except urllib.request.URLError as err:
            if isinstance(err.reason, socket.timeout):
                raise TimeOutError() from None
            else:
            ...
            if err.code == 500:
                ...
                for child in xml.iter():
                    if child.tag.endswith("errorCode") and child.text == "600":
                        raise SOAPError("Invalid PIN Code!") from None
                    elif child.tag.endswith("errorDescription"):
                        raise SOAPError(child.text) from None
            elif err.code == 403:
                 raise ForbiddenError() from None
            raise err from None # Pass to the next handler
        except urllib.request.URLError as err:
            if isinstance(err.reason, socket.timeout):
                raise TimeOutError() from None
            else:
    ...
    def _do_custom_request(self, method, url, headers=None):
        opener = urllib.request.build_opener(urllib.request.HTTPHandler)
        req = urllib.request.Request(url, headers=headers, method=method)
        res = opener.open(req, timeout=self._timeout)
    ...
    def upnp_service_subscribe(self, service):
        """Subscribe to a UPnP service."""
        headers = {
            "NT": "upnp:event",
            "TIMEOUT": "Second-" + str(self._timeout),
            "HOST": f"{self._host}:{self._port}",
    ...
    def upnp_service_resubscribe(self, service):
    ...
    def upnp_service_unsubscribe(self, service):
    ...
    async def async_stop_server(self):
        ...
        if self._aiohttp_server:
            await self._aiohttp_server.shutdown()
